...
let draggables: string[] = [];
let thresholdIndicator: HTMLElement | null = null;
...
function handleWait(element: HTMLElement): void {
    if (!socket || socket.readyState !== WebSocket.OPEN) {
        console.error("WebSocket is not open");
        initializeWebSocket();
        return;
    }
    const durationInput = element.querySelector(
        ".duration-input"
    ) as HTMLInputElement;
    try {
...
function handleStop(element: HTMLElement): void {
    if (!socket || socket.readyState !== WebSocket.OPEN) {
        console.error("WebSocket is not open");
        initializeWebSocket();
        return;
...
function handleDraggable1Send(element: HTMLElement): void {
    ...
    const directionSelect = element.querySelector(
        ".direction-select"
    ...
    try {
        const duration = parseInt(durationInput?.value || "1");
        const direction = directionSelect?.value || 1;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (commandButton) {
        commandButton.addEventListener("click", async (event) => {
            ...
            // Find START elements
            const startElements = draggables.filter((id) =>
                id.startsWith("draggable2-")
            );
            ...
            for (const startElementId of startElements) {
                ...
                console.log(
                    `Found ${belowElements.length} elements below START`
                );
...
function isNearPosition(
    ...
    return (
        Math.abs(pos1.x - pos2.x) < targetWidth &&
...
function checkPositionOccupied(
    snapPos: Position,
    element: HTMLElement,
    excludeIds: string[]
): HTMLElement | null {
...
function handleElementSnapping(
...
function trackDraggableElement(id: string): void {
...
function showMousePosition(x: number, y: number): void {
    ...
    positionIndicator.textContent = `X: ${Math.round(x)}px, Y: ${Math.round(
        y
    )}px`;
...
document.addEventListener("dragover", (event: DragEvent) => {
    ...
    const snapTargets = draggables
        ...
        .filter((element): element is HTMLElement => element !== null)
        .filter((other) => {
            if (other === currentDraggedElement) return false;
        ...
        .map((other) => {
            ...
            const horizontalDistance = Math.abs(
                draggedRect.left - otherRect.left
            );
        ...
        .filter(({ element, horizontalDistance, verticalDistance }) => {
            ...
            return (
                horizontalDistance < 110 && verticalDistance < otherRect.height
            );
        });
...
function showThresholdArea(
    element: HTMLElement,
    aboveElement: HTMLElement
): void {
...
function checkAndAdjustPositions(changedElement: HTMLElement | null = null) {
    ...
    if (nocolseelements == true) {
        console.error(
            "Některé bloky se nepodařilo správně umístit, prosím přesuňte je ručně."
        );
        nocolseelements = false;
    }
}
...
function processElement(
    ...
    const CloseElements = allDraggables
        ...
        .map((other) => {
            ...
            const horizontalDistance = Math.abs(
                elementRect.left - otherRect.left
            );
        ...
        .filter(({ element, horizontalDistance, verticalDistance }) => {
            const otherRect = element.getBoundingClientRect();
            return (
                horizontalDistance < otherRect.width && verticalDistance < otherRect.height
            );
        })
        .sort((a, b) => {
    ...
    const processedHeight = processedElementsInLine.reduce((height, el) => {
        ...
        const occupyingElements = allDraggables
            .filter(other => {
                if (other === element || processedElements.has(other.id)) return false;
                const otherRect = other.getBoundingClientRect();
